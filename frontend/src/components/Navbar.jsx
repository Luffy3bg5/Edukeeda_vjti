...
const Navbar = () => {
  ...
  return (
    <>
    <header className="fixed top-0 left-0 right-0 h-16 bg-[#090514]/80 backdrop-blur-xl border-b border-white/5 z-50 flex items-center justify-between px-4 md:px-8">
      {/* Brand */}
      ...
      <div className="flex-1 max-w-xl mx-4 lg:mx-12 hidden md:flex items-center gap-3 relative pointer-events-auto" ref={dropdownRef}>
        <div className="w-full relative">
          <form onSubmit={handleSearchSubmit} className="relative flex items-center bg-[#181024] border border-white/10 rounded-full overflow-hidden focus-within:ring-2 focus-within:ring-violet-500/50 transition">
          ...
          {suggestions.length > 0 && (
            <div className="absolute top-12 left-0 right-0 bg-[#181024] border border-white/10 shadow-2xl rounded-2xl overflow-hidden py-2 z-50">
              {suggestions.map((item) => (
                ...
                >
                  ...
                  <span className="text-xs text-violet-400 font-medium uppercase tracking-wider">{item.type}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <button 
          ...
          className="flex-shrink-0 flex items-center gap-2 px-5 py-2.5 bg-violet-600/20 hover:bg-violet-600/40 text-violet-100 border border-violet-500/50 hover:border-violet-400 rounded-full transition shadow-sm font-bold"
        ...
        {user ? (
          <div className="flex items-center gap-4">
             <div className="hidden sm:flex flex-col items-end">
               ...
               <span className="text-xs text-violet-400 font-medium uppercase tracking-wider">{user.role}</span>
             </div>
             <button 
               onClick={() => setIsProfileOpen(true)}
               className="w-10 h-10 rounded-full bg-gradient-to-br from-rose-500 to-violet-600 flex items-center justify-center text-white font-bold shadow-lg hover:scale-105 transition"
               title="Profile"
             >
    ...
    {isProfileOpen && user && (
      <div className="fixed inset-0 z-[100] flex justify-end bg-black/60 backdrop-blur-sm pointer-events-auto">
        <div className="w-full max-w-2xl bg-[#090514] h-full overflow-y-auto border-l border-white/10 shadow-2xl transition-transform duration-300 relative">
            <div className="sticky top-0 z-50 flex justify-between items-center p-6 border-b border-white/10 bg-[#090514]/80 backdrop-blur-md">
