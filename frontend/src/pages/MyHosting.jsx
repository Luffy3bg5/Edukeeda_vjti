...
const MyHosting = () => {
  ...
  return (
    <div className="p-6 md:p-10 max-w-5xl mx-auto space-y-8">
      <div className="bg-[#181024] border border-white/5 rounded-3xl p-8 relative overflow-hidden shadow-2xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center relative z-10 gap-6 mb-8 border-b border-white/10 pb-6">
          ...
          <Link to="/host-event" className="flex items-center gap-2 bg-gradient-to-r from-violet-600 to-rose-600 text-white px-6 py-3 rounded-xl font-bold hover:shadow-[0_0_20px_rgba(139,92,246,0.4)] transition-all">
        ...
        {loading ? (
            ...
            {requests.map((req, index) => {
              ...
              return (
                <motion.div 
                  ...
                  className="bg-[#090514] p-6 rounded-2xl border border-white/5 shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-6 hover:shadow-xl hover:border-white/10 transition-all"
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <span className="px-3 py-1 bg-violet-500/20 text-violet-300 rounded-lg text-xs font-black uppercase tracking-wider">{req.eventType}</span>
                  ...
                  <div className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm ${statusConfig.status === 'pending' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : req.status === 'approved' ? 'bg-violet-500/10 text-violet-400 border border-violet-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'} whitespace-nowrap`}>
          ...
          <div className="text-center py-20 bg-[#090514] rounded-3xl border border-dashed border-white/10 relative z-10">
            <FilePlus className="w-16 h-16 text-violet-500/40 mx-auto mb-6" />
            ...
            <Link to="/host-event" className="inline-flex items-center gap-2 bg-gradient-to-r from-violet-600 to-rose-600 text-white px-8 py-4 rounded-xl font-bold hover:shadow-[0_0_20px_rgba(139,92,246,0.4)] transition-all">
              Start Hosting
            </Link>
          </div>
        )}
